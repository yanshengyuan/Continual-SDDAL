...
# ==========================================
# SDDAL_l2.sh — Replay + L2 weight anchor variant of SDDAL.sh
#
# Identical to SDDAL_ting.sh except the AL loop uses L2Trainer.py
# (warm-start + experience replay + L2 weight anchor) instead of
# ContinualTrainer.py (warm-start + experience replay only).
#
# Key differences vs SDDAL_ting.sh (replay):
#   - AL loop: L2Trainer.py instead of ContinualTrainer.py
#   - exp_path suffix: _l2  (vs _cl for replay)
#   - New arg 15: beta (L2 penalty weight, default 0.01)
#
# The L2 anchor penalises the model for drifting away from the previous
# round's checkpoint weights:
#   L = QuantileLoss + beta * ||theta - theta_prev||^2
# theta_prev is always the most recent checkpoint — never stale.
#
# Usage:
#   bash SDDAL_l2.sh <beamshape> <lr> <initial_size> <fix_init?> <init_only?> <start_round> <end_round> <gpu> <scanner_batch_size> <retrain_frequency> <scan_only?> <seed> <init_seed> <buffer_size> <beta>
#
# ==========================================
# Examples:
#
# 1) L2 full pipeline  -> writes to Design_rec_1_l2/
#    bash SDDAL_l2.sh rec 0.0002 100 true false 1 100 0 5 1 false 1 123 300 0.01
#
# 2) Resume
#    bash SDDAL_l2.sh rec 0.0002 100 true false 41 100 0 5 1 false 1 123 300 0.01
#
# ==========================================
...
exp_path="Design_${beamshape}_${seed}_l2"
...
echo "========================================="
echo " SDDAL L2 Pipeline (L2Trainer)"
...
do
    ...
    if [ "${scan_only}" = false ]; then
        if (( (round_sampling-1) % retrain_freq == 0 )); then
            echo "------------------------------"
            echo "  L2Trainer at round ${round_sampling}"
            echo "  (warm-start + replay + L2 anchor, every ${retrain_freq} scans)"
            echo "------------------------------"
...
wall_clock_s=$(( $(date +%s) - loop_start ))
echo "========================================="
echo "   SDDAL L2 Pipeline Completed Successfully!"
