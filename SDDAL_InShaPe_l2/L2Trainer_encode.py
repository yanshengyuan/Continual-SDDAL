...
round's checkpoint:
    ...
    L = QuantileLoss(low, mu, high, target)       [task loss, all samples]
      + beta * sum_i (theta_i - theta_prev_i)^2   [L2 anchor, all params]
...
parser.add_argument('--beta', default=0.01, type=float,
                    help='Weight of L2 anchor penalty. Penalises deviation '
                         'from the previous checkpoint weights.')
...
def main():
    ...
    print(f'[L2Trainer] Training on {len(new_I_paths)} new + '
          f'{len(buf_I)} buffer = {len(combined_I)} samples | beta={args.beta}')
    ...
    for epoch in range(args.epochs):
        train_epoch(loader, model, criterion, optimizer, epoch, device,
                    args.print_freq, prev_params, args.beta)
        scheduler.step()
