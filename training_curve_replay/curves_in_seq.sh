#!/bin/bash

# Usage:
#   bash run_all_beams.sh <data_root>
# Example:
#   bash run_all_beams.sh ../SDDAL_InShaPe_randinit_syncUNet_varseed

DATA_ROOT="$1"

if [ -z "$DATA_ROOT" ]; then
    echo "Usage: $0 <data_root>"
    exit 1
fi

wait_until_beam_done () {
    local beam="$1"

    while pgrep -f "python3 retrain.py .*Design_${beam}" > /dev/null; do
        echo "$(date '+%F %T')  Waiting for ${beam} jobs to finish..."
        sleep 20
    done
}

run_training () {
    local beam="$1"
    local lr="$2"
    local seed="$3"

    echo "=================================================="
    echo "$(date '+%F %T')  Launching training: beam=${beam}, lr=${lr}"
    echo "=================================================="

    bash TrainSet_curve_sddal.sh "$beam" "$lr" "$DATA_ROOT" "$seed"

    echo "$(date '+%F %T')  All ${beam} jobs launched. Now waiting..."
    wait_until_beam_done "$beam"
    echo "$(date '+%F %T')  ${beam} training finished."
    echo
}

run_eval () {
    local beam="$1"

    echo "$(date '+%F %T')  Running eval for ${beam}..."

    pushd "$beam" > /dev/null || {
        echo "$(date '+%F %T')  Failed to enter folder: $beam"
        return 1
    }

    bash eval_all.sh

    popd > /dev/null || {
        echo "$(date '+%F %T')  Failed to return to project root"
        return 1
    }

    echo "$(date '+%F %T')  ${beam} eval launched."
    echo
}

# ── Phase 1: training (sequential, one seed at a time) ──
run_training rec_1_500 0.0002 9999
run_training rec_2_500 0.0002 9999
run_training rec_3_500 0.0002 9999
#run_training rec_4_500 0.0002 9999
#run_training rec_5_500 0.0002 9999

# ── Phase 2: evaluation (all seeds launched together) ──
run_eval rec_1_500
run_eval rec_2_500
run_eval rec_3_500
#run_eval rec_4_500
#run_eval rec_5_500

echo "$(date '+%F %T')  All beamshapes finished."