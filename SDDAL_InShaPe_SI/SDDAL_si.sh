...
# ==========================================
# SDDAL_si.sh — Replay + Synaptic Intelligence (SI) variant of SDDAL.sh
#
# Identical to SDDAL_ting.sh except the AL loop uses SITrainer.py:
#   - Warm-start from previous checkpoint
#   - Experience replay
#   - SI regularisation: (si_lambda/2) * sum_i Omega_i * (theta_i - theta_prev_i)^2
#     Omega accumulates across ALL rounds; anchor moves each round.
#
# exp_path suffix: _si
# New arg 15: si_lambda (SI regularisation strength, default 1.0)
#
# Usage:
#   bash SDDAL_si.sh <beamshape> <lr> <initial_size> <fix_init?> <init_only?> <start_round> <end_round> <gpu> <scanner_batch_size> <retrain_frequency> <scan_only?> <seed> <init_seed> <buffer_size> <si_lambda>
#
# ==========================================
# Examples:
#
# 1) SI full pipeline  -> writes to Design_rec_1_si/
#    bash SDDAL_si.sh rec 0.0002 100 true false 1 200 0 5 1 false 1 123 300 1.0
#
# 2) Resume at round 41
#    bash SDDAL_si.sh rec 0.0002 100 true false 41 200 0 5 1 false 1 123 300 1.0
#
# ==========================================
...
exp_path="Design_${beamshape}_${seed}_si_${si_lambda}"

buffer_path="${exp_path}/models/replay_buffer.pkl"
si_state_path="${exp_path}/models/si_state.pkl"
...
echo "========================================="
echo " SDDAL SI Pipeline (SITrainer)"
...
echo " SI state path        : ${si_state_path}"
echo " SI lambda            : ${si_lambda}"
...
do
    ...
    if [ "${scan_only}" = false ]; then
        if (( (round_sampling-1) % retrain_freq == 0 )); then
            echo "------------------------------"
            echo "  SITrainer at round ${round_sampling}"
            echo "  (warm-start + replay + SI, every ${retrain_freq} scans)"
            echo "------------------------------"
...
wall_clock_s=$(( $(date +%s) - loop_start ))
echo "========================================="
echo "   SDDAL SI Pipeline Completed Successfully!"
