...
# ==========================================
# SDDAL_der.sh — DER variant of SDDAL.sh
#
# Identical to SDDAL_ting.sh except the AL loop uses DERTrainer.py
# (warm-start + experience replay + distillation loss) instead of
# ContinualTrainer.py (warm-start + experience replay only).
#
# Key differences vs SDDAL_ting.sh (replay):
#   - AL loop: DERTrainer.py instead of ContinualTrainer.py
#   - exp_path suffix: _der  (vs _cl for replay)
#   - buffer file:     der_buffer.pkl  (vs replay_buffer.pkl)
#   - New arg 15:      alpha (distillation loss weight, default 0.5)
#
# Usage:
#   bash SDDAL_der.sh <beamshape> <lr> <initial_size> <fix_init?> <init_only?> <start_round> <end_round> <gpu> <scanner_batch_size> <retrain_frequency> <scan_only?> <seed> <init_seed> <buffer_size> <alpha>
#
# ==========================================
# Examples:
#
# 1) DER full pipeline  -> writes to Design_rec_1_der/
#    bash SDDAL_der.sh rec 0.0002 100 true false 1 200 0 5 1 false 1 123 300 0.5
#
# 2) Init-only
#    bash SDDAL_der.sh rec 0.0002 100 true true 1 200 0 5 1 false 1 123 300 0.5
#
# 3) Resume DER pipeline
#    bash SDDAL_der.sh rec 0.0002 100 true false 41 200 0 5 1 false 1 123 300 0.5
#
# ==========================================
...
exp_path="Design_${beamshape}_${seed}_der_${alpha}"

buffer_path="${exp_path}/models/der_buffer.pkl"
...
echo "========================================="
echo " SDDAL DER Pipeline (DERTrainer)"
...
echo " DER buffer size      : ${buffer_size}"
...
if [ "${start_round}" -eq 1 ] && [ "${scan_only}" = false ]; then
    ...
    echo "  Seeding DER buffer: n_trained=${init_size} ..."
    python3 -c "
...
buf = DERBuffer(max_size=${buffer_size})
buf.n_trained = ${init_size}
buf.save('${buffer_path}')
print(f'[der_buffer] n_trained={buf.n_trained}, max_size={buf.max_size} -> ${buffer_path}')
"
fi
...
do
    ...
    if [ "${scan_only}" = false ]; then
        if (( (round_sampling-1) % retrain_freq == 0 )); then
            echo "------------------------------"
            echo "  DERTrainer at round ${round_sampling}"
            echo "  (warm-start + replay + distillation, every ${retrain_freq} scans)"
            echo "------------------------------"
...
wall_clock_s=$(( $(date +%s) - loop_start ))
echo "========================================="
echo "   SDDAL DER Pipeline Completed Successfully!"
